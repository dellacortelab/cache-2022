...
orig_dir = os.path.join(base_dir, '3_cleaned_sdf_top_1500')
intermediate_dir = os.path.join(base_dir, '3_5_md_in_progress')
final_dir = os.path.join(base_dir, '4_already_run')
output_dir = os.path.join(base_dir, '5_md_runs')
...
while True:
    ...
    for taskID in range(1, 4):
        ret = os.system(f'python -u ./complex_test.py -s {intermediate_file} --gpu {args.gpu} --length 50 --iter {taskID} --out {sdf_output_dir}')
